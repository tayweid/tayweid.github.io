...
Throughout this unit, we build custom maximum likelihood estimation routines in Python using `scipy.optimize`. Writing the log-likelihood by hand provides a deeper understanding of how these models actually work under the hood.
...
To make the framework concrete, we examine the travel choices for 3,880 travellers between Montreal and Toronto. The transportation options are car, bus, train, and air.
...
url = "https://raw.githubusercontent.com/tayweid/tayweid.github.io/master/econ-2823/parts/part-15/ModeCanada.csv.gz"
...
Note that `statsmodels.MNLogit` implements only the basic multinomial logit where covariates vary only across individuals, not across alternatives. To implement the full conditional logit (McFadden's choice model), we write a custom log-likelihood function and maximize it with `scipy.optimize`.
...
Multinomial probit estimation is computationally intensive due to the simulation required. The custom implementation above demonstrates the core mechanics.
...
- These models require writing custom log-likelihood functions and using `scipy.optimize`.
