...
The Nelder-Mead method does not use gradients at all. Instead, it constructs a simplex (a triangle in 2D, a tetrahedron in 3D) and iteratively deforms it to explore the function landscape:
...
- SciPy's `optimize.minimize` is the Python workhorse for optimization
