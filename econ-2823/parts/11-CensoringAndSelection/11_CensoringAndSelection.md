...
Throughout this unit, we implement these models from scratch in Python using custom log-likelihood functions and `scipy.optimize`. Writing the log-likelihood by hand reinforces understanding of how censoring and selection affect estimation.
...
Historically (1970s), two-step estimators were used because of computational limitations. As noted in the original literature: "The original article suggests using the two-step solution for exploratory work and as initial values for ML estimation, since in those days the cost of the two-step solution was \$15 while that of the maximum-likelihood solution was \$700."
...
- These models are implemented as custom MLE routines using `scipy.optimize`.
