...
c) Create a faceted line plot showing coffee production over time, with each country in its own panel. (*Hint: use `sns.relplot()` with `col='Entity'` and `col_wrap=4`.*)
