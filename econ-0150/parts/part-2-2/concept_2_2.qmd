...
```{.python}
# Boxplot
sns.boxplot(data, x='Offer ID', y='Revenue')
```
...
```{.python}
sns.boxplot(data, x='Offer ID', y='log2_Revenue')
```
...
<p class="subheader">Count the unique values in the Event column to understand what's causing the zeros.</p>
...
```{.python}
data['Event'].value_counts().plot(kind='barh')
```
...
```{.python}
sns.boxplot(transactions, x='Offer ID', y='log2_Revenue')
...
sns.boxplot(two_offers, x='Offer ID', y='log2_Revenue')
```
