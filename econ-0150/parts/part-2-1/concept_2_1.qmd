...
survey['birthday'] = pd.to_datetime(survey['Approximately how many miles away from Pittsburgh is your hometown?'])
```
...
survey_dropped = survey.dropna(subset=['distance_clean'])
```
...
median_dist = survey['distance_clean'].median()
survey['distance_filled'] = survey['distance_clean'].fillna(median_dist)
```
...
```{.python}
# Or replace with 0
survey['distance_zero'] = survey['distance_clean'].fillna(0)
```
...
```{.python}
# Create scatterplot
sns.scatterplot(data=survey_dropped, x='distance_clean', y='age')
```
