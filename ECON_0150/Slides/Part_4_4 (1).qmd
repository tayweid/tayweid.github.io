...
<p class="subheader">How is neighborhood income related to air quality?</p>
...
data = pd.read_csv(file_path + "population_demographics.csv")
```
...
model1 = smf.ols('air_pollution_index ~ I(median_income > 100000)', data=data).fit()
```
...
model2 = smf.ols('air_pollution_index ~ I(median_income < 50000) + I(median_income > 120000)', data=data).fit()
```
...
model3 = smf.ols('air_pollution_index ~ C(region)', data=data).fit()
```
...
<p class="subheader">How are education and wages related?</p>
...
plt.scatter(education, wages, alpha=0.6, color='#4C72B0')
...
# Annotate the slope
plt.annotate(f'Slope ≈ ${slope:.0f} per year', 
             xy=(16, intercept + 16*slope), 
             xytext=(16, intercept + 16*slope - 10000),
             arrowprops=dict(facecolor='black', shrink=0.05, width=1.5),
             fontsize=12)

plt.title('Relationship Between Education and Wages', fontsize=14)
...
<p class="subheader">Not everything follows a straight line</p>
...
# Fit multiple regression model
model = smf.ols('wages ~ education + experience', data=data).fit()
```
...
## Looking Forward
<p class="subheader">Next steps in our regression journey</p>
