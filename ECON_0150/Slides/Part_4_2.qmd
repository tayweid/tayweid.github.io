...
# Plot the histogram of sample means
plt.figure(figsize=(10, 4))
plt.hist(sample_means, bins=30, color='skyblue', alpha=0.1, density=True, 
         edgecolor='white')
...
# Add vertical line for true mean
plt.axvline(true_mean, color='green', linestyle='--', linewidth=2, label='Sample Mean')
...
plt.axvline(4, color='black', linestyle='--', alpha=0.2, linewidth=1, label='Equally Extreme')
...
```python
# Run the model
model = sm.OLS(data, np.ones(len(diff))).fit()
...
plt.figure(figsize=(10, 4))
# Plot histogram of slope coefficients
plt.hist(slope_coefficients, bins=30, alpha=0.1, density=True, 
