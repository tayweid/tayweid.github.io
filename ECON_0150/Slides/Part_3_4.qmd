...
<p class="subheader">We used the distribution of sample means to create confidence intervals around $\bar{x}$.</p>
...
- 95% of samples will have $\bar{x}$ and $\mu$ roughly 1.96 standard errors apart 
:::
...
plt.title(f'Distribution of Sample Means (n={sample_size})', fontsize=20)
plt.yticks([])
...
$$\bar{x} \sim t_{29}\left(10, \frac{2.5}{\sqrt{30}}\right)$$

$$\bar{x} \sim t_{29}(10, 0.456)$$

A 95% confidence interval around $\mu_0$ would be:

$$[10 - 2.045 \times 0.456, 10 + 2.045 \times 0.456]$$
...
<p class="subheader">Use the sampling distribution to find the probability.</p>
...
*> interpretation: if μ=10, we'd see $\bar{x}$ this far from 10 about 6.4% of the time*
